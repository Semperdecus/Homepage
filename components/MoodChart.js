...
const monthlyAnnotations = (data) => data.reduce((annotations, entry, index) => {
  ...
  if (index === 0 || new Date(data[index - 1].date).getMonth() !== currentMonth) {
    // Add an annotation for the start of the month (dotted line)
    annotations.push({
      type: "line",
      xMin: entry.date,
      xMax: entry.date,
      ...
      label: {
        content: `Start of ${currentDate.toLocaleString("default", { month: "long" })}`,
        position: "center",
...
export default function MoodChart({data}) {
  ...
  const chartOptions = {
    responsive: true,
    maintainAspectRatio: false, // Keeps the aspect ratio while scaling
    scales: {
      x: {
        ticks: {
          callback: function (val, index) {
            // Get the date corresponding to the current index
            const date = new Date(this.getLabelForValue(val));
            // Check if it's the first day of the month
            if (date.getDate() === 1) {
              // Return the abbreviated month name (e.g., "Jan", "Feb", "Mar")
              return date.toLocaleString('default', { month: 'short' });
            }

            // For other dates, return an empty string (hide them)
            return '';
          },
    ...
    plugins: {
      annotation: {
        annotations: monthlyAnnotations(data), // Add the dynamic annotations array
      },
    },
  };
