...
import highlightDataUnfiltered from '../components/YearOverview/highlights/highlights2025.json';
import photoBackgroundPythonScript from '../components/YearOverview/photoBackground/photoBackground2025.json';
...
export default function Index() {
  ...
  useEffect(() => {
    fetchAndParseCsv("/files/daylio_export_2025.csv").then((response) => {
      return response;
    })
      .then((parsedData) => {
        const processedMoodData = processMoodData(parsedData.reverse(), 2025);
  ...
  const randomizedBackgroundPhotoData = () => {
    return [...photoBackgroundPythonScript, ...highlightDataUnfiltered].sort(() => Math.random() - 0.5); // Shuffle the array randomly
  };
  ...
  return (
    ...
    >
      <div style={{
        ...
        <Options onIconClick={handleIconClick} activeYear={2025}/>
